...
каждого. Регистрируешь приложение на [dev.hh.ru](https://dev.hh.ru) и меняешь
Client ID/Secret на бессрочный токен: это токен приложения, а не личный, доступ
к твоим откликам и резюме он не даёт. Минут пять и один раз — по шагам в
[docs/setup.md](docs/setup.md). Кладётся в `.env` как `HH_TOKEN`, после чего hh
включается галочкой в пульте; до этого он там виден с пометкой «нужен HH_TOKEN».

Без токена источник просто выключен, остальные 18 работают. Смысл возиться в
том, что через hh видны компании с закрытыми сайтами (Сбер, Контур, Озон) —
почти все публикуют вакансии туда.
