...
Про hh.ru: анонимный доступ к API закрыт, нужен бесплатный токен — свой, у
каждого. И это не «нажал кнопку и получил»: на [dev.hh.ru](https://dev.hh.ru)
подаётся заявка на регистрацию приложения, её смотрят люди — обещают ответ за
несколько рабочих дней, в редких случаях до 15, и могут отказать. Поэтому hh —
источник на вырост, а не шаг установки: ставь sasaki без него, остальные 18
работают сразу.

Если заявку одобрили, Client ID/Secret меняются на бессрочный токен (по шагам —
[docs/setup.md](docs/setup.md)). Это токен приложения, а не личный: доступа к
твоим откликам и резюме он не даёт. Кладётся в `.env` как `HH_TOKEN`, после чего
hh включается галочкой в пульте; до этого он там виден с пометкой
«нужен HH_TOKEN».

Смысл возиться в том, что через hh видны компании с закрытыми сайтами (Сбер,
Контур, Озон) — почти все публикуют вакансии туда.
