...
Нужен только **Node.js ≥ 21**. Мастер спросит, кого ищешь и куда доставлять
находки, поставит автозапуск и сделает первый прогон. Дальше открываешь
`http://127.0.0.1:4321` — пульт уже работает и будет работать всегда.
...
Мастер настройки предлагает поставить его сразу. Отдельно — так:
