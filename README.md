...
Про hh.ru: читается публичная RSS-лента поиска — без токена, без логина и без
регистрации, как и всё остальное. Это важный источник: через него видны
компании, чьи сайты закрыты бот-протекшенами (Сбер, Контур, Озон) — почти все
публикуют вакансии туда.

Их официальное API мы не используем: анонимный доступ к вакансиям закрыт, а
токен выдают только по заявке с модерацией нескольких отделов — до 15 рабочих
дней по их же FAQ. RSS отдаёт 20 свежих вакансий на запрос, и для поиска работы
раз в 6 часов этого хватает с запасом.
